...
export default function DatabaseView({ 
  ...
  const [editorMode, setEditorMode] = useState<'side-panel' | 'modal' | 'full-page' | undefined>(undefined)
  ...
  const handleAddItem = useCallback((initialValues?: Record<string, any>) => {
  ...
  return (
    <div className="flex-1 h-screen flex flex-col bg-graphon-bg dark:bg-[#191919] overflow-hidden text-graphon-text-main dark:text-[#dfdfdf] font-sans relative">
      ...
      <div className={`flex-1 flex flex-col transition-all duration-500 ease-apple ${selectedItem ? 'blur-[3px] scale-[0.985] brightness-[0.9] pointer-events-none' : ''}`}>
        {/* Database Header */}
        <div className="flex-shrink-0 px-8 pt-10 pb-4">
          <div className="flex items-center gap-3">
            <span className="text-3xl">{database.icon}</span>
            <h1 className="text-3xl font-bold text-graphon-text-main dark:text-[#dfdfdf] tracking-tight">{database.name}</h1>
          </div>
        </div>
      ...
      {selectedItem && (
        <NewItemEditor
          ...
          isFavorite={favorites.some(f => f.databaseId === database.id && f.itemId === selectedItem.id)}
        />
      )}
    </div>
  )
