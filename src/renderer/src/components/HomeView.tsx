...
export default function HomeView({ 
  ...
  const [homeDatabase, setHomeDatabase] = useState<{ name: string, items: Item[], id: string } | null>(null)
  ...
  const templates = [
    {
      ...
      imageColor: 'bg-gradient-to-br from-orange-100 to-orange-200 dark:from-orange-900/40 dark:to-orange-800/20',
    ...
    },
    {
      ...
      imageColor: 'bg-gradient-to-br from-blue-100 to-blue-200 dark:from-blue-900/40 dark:to-blue-800/20',
    ...
    },
    {
      ...
      imageColor: 'bg-gradient-to-br from-green-100 to-green-200 dark:from-green-900/40 dark:to-green-800/20',
    ...
    },
    {
      ...
      imageColor: 'bg-gradient-to-br from-purple-100 to-purple-200 dark:from-purple-900/40 dark:to-purple-800/20',
      ...
      <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-100">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          ...
          >
             ...
             <div className="z-10">
               <span className="text-sm font-medium text-graphon-text-main dark:text-white block">New page</span>
      ...
      <section className="animate-in fade-in slide-in-from-bottom-4 duration-500 delay-200">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {templates.map(template => (
            <div 
              key={template.id}
              onClick={() => handleCreateFromTemplate(template.id, template.title, template.description)}
            ...
            >
              ...
              <div className={`h-32 w-full ${template.imageColor} flex items-center justify-center relative overflow-hidden`}>
          ...
          <div className="bg-white dark:bg-zinc-800/30 rounded-xl border border-graphon-border dark:border-white/5 flex flex-col h-[400px]">
             ...
             <div className="p-4 border-b border-graphon-border/50 dark:border-white/5 flex items-center justify-between">
                ...
                <div className="flex space-x-1">
                    <button onClick={() => setSelectedDate(addDays(selectedDate, -7))} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-700/50 rounded">
                        <ChevronLeftIcon className="w-4 h-4 text-zinc-500" />
                    </button>
                    <button onClick={() => setSelectedDate(new Date())} className="text-xs font-medium px-2 py-1 hover:bg-zinc-100 dark:hover:bg-zinc-700/50 rounded">
                        Today
                    </button>
                    <button onClick={() => setSelectedDate(addDays(selectedDate, 7))} className="p-1 hover:bg-zinc-100 dark:hover:bg-zinc-700/50 rounded">
             ...
             <div className="flex justify-between px-4 py-2 bg-zinc-50/50 dark:bg-zinc-900/20 text-xs font-medium text-zinc-500 dark:text-zinc-400">
                  {['M','T','W','T','F','S','S'].map((d, i) => <div key={i} className="w-8 text-center">{d}</div>)}
             </div>
                ...
                {displayedEvents.length === 0 ? (
                    ...
                    <div className="space-y-2">
                        {displayedEvents.map(event => (
                            <div key={event.id} className="flex flex-col px-3 py-2 bg-white dark:bg-zinc-800/80 rounded-lg border border-graphon-border/50 dark:border-white/5 shadow-sm">
                                <span className="text-sm font-medium text-graphon-text-main dark:text-white">{event.title}</span>
               ...
               <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                  ...
                  {!isSelectingDatabase ? (
                      ...
                      <div className="w-full max-w-xs animate-in fade-in slide-in-from-bottom-2">
                          <p className="text-xs font-semibold text-zinc-500 uppercase mb-2 text-left">Select to pin</p>
                ...
                <div className="flex-1 flex flex-col min-h-0">
                   <div className="p-4 border-b border-graphon-border/50 dark:border-white/5 flex items-center justify-between flex-shrink-0">
                      <div className="flex items-center space-x-2">
                          <span className="text-lg">{homeDatabase.icon || '📋'}</span> {/* We don't have icon in minimal state, just use default */}
                          <h3 className="font-medium text-graphon-text-main dark:text-white">{homeDatabase.name}</h3>
                      </div>
                      <div className="flex items-center space-x-2">
                        <button 
                            onClick={() => {
                   ...
                   <div className="p-2 space-y-1 overflow-auto flex-1 h-full">
                      {homeDatabase.items.length === 0 ? (
                          ...
                          homeDatabase.items.map(item => (
                             <div key={item.id} className="flex items-center justify-between px-3 py-2 hover:bg-zinc-50 dark:hover:bg-zinc-700/30 rounded-lg transition-colors cursor-default border border-transparent hover:border-zinc-200 dark:hover:border-zinc-700/50">
                                ...
                                {item.values?.Status && (
                                   <span className={`
                                        ...
                                        ${item.values.Status === 'Done' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 
                                          item.values.Status === 'In Progress' ? 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400' :
